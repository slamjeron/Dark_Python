    ...
    if isinstance(x, collections.Sequence):
        if len(x) == 2:
            if y is None:
                ...
                raise ValueError(
                    'When passing a sequence at the x argument, the y argument must not be passed (received {0}).'.format(
                        repr(y)))
        else:
...
def displayMousePosition(xOffset=0, yOffset=0):
    ...
    try:
        while True:
            ...
            if (x - xOffset) < 0 or (y - yOffset) < 0 or (x - xOffset) >= resolution[0] or (y - yOffset) >= resolution[
                1]:
